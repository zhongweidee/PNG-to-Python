...
Super simple method for converting a set of png images (save as black and white mode) into mnist binary format for training (depends on imagemagick and python 2.7 PIL)
...
```bash
sudo apt-get update -- no need for windows
sudo apt-get install imagemagick php5-imagick  -- no need for windows
pip install pillow
```
